...
fetch last from cursor_products into cursor_record;
        loop
            EXIT WHEN NOT FOUND;
            RAISE NOTICE 'ID: %, Name: %, Count: %, Price: %',
            cursor_record.id, cursor_record.name, cursor_record.count, cursor_record.price;
            FETCH prior from cursor_products into cursor_record;
        END LOOP;
